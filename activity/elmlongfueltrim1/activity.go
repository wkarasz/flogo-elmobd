...
var log = logger.GetLogger("activity-elm-LongFuelTrim1")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewLongFuelTrim1())

	if err != nil {
                log.Infof("Failed to get long fuel trim 1 [%s]", err)
                return
        }

        log.Infof("Long Fuel Trim 1 is [%s]", elmObj.ValueAsLit())
