...
var log = logger.GetLogger("activity-elm-throttleposition")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewThrottlePosition())

	if err != nil {
                log.Infof("Failed to get throttle position [%s]", err)
                return
        }

        log.Infof("Throttle position is [%s]", elmObj.ValueAsLit())
