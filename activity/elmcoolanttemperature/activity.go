...
var log = logger.GetLogger("activity-elm-CoolantTemperature")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewCoolantTemperature())

	if err != nil {
                log.Infof("Failed to get coolant temperature [%s]", err)
                return
        }

        log.Infof("Coolant temperature [%s]", elmObj.ValueAsLit())
