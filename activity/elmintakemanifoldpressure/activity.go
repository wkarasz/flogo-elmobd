package elmintakemanifoldpressure
...
var log = logger.GetLogger("activity-elm-IntakeManifoldPressure")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewIntakeManifoldPressure())

	if err != nil {
                log.Infof("Failed to get intake manifold pressure [%s]", err)
                return
        }

        log.Infof("Intake manifold pressure is [%s]", elmObj.ValueAsLit())
