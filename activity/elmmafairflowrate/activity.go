...
var log = logger.GetLogger("activity-elm-mafairflowrate")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewMafAirFlowRate())

	if err != nil {
                log.Infof("Failed to get MAF Airflow rate [%s]", err)
                return
        }

        log.Infof("MAF airflow rate is [%s]", elmObj.ValueAsLit())
