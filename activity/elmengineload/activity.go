...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewEngineLoad())

	if err != nil {
                log.Infof("Failed to get engine load [%s]", err)
                return
        }

        log.Infof("Engine load [%s]", elmObj.ValueAsLit())
