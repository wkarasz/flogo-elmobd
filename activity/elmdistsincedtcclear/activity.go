...
var log = logger.GetLogger("activity-elm-DistSinceDTCClear")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewDistSinceDTCClear())

	if err != nil {
                log.Infof("Failed to get [%s]", err)
                return
        }

        log.Infof("Details [%s]", elmObj.ValueAsLit())
