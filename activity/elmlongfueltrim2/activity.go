...
var log = logger.GetLogger("activity-elm-LongFuelTrim2")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewLongFuelTrim2())

	if err != nil {
                log.Infof("Failed to get long fuel trim 2 [%s]", err)
                return
        }

        log.Infof("Long Fuel Trim 2 is [%s]", elmObj.ValueAsLit())
