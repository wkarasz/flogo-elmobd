...
var log = logger.GetLogger("activity-elm-TimingAdvance")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewTimingAdvance())

	if err != nil {
                log.Infof("Failed to get timing advance [%s]", err)
                return
        }

        log.Infof("Timing advance is [%s]", elmObj.ValueAsLit())
