...
var log = logger.GetLogger("activity-elm-runtimesincestart")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewRuntimeSinceStart())

	if err != nil {
                log.Infof("Failed to get runtime since start [%s]", err)
                return
        }

        log.Infof("Runtime since start is [%s]", elmObj.ValueAsLit())
