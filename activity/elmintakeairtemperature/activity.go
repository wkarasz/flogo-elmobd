package elmintakeairtemperature
...
var log = logger.GetLogger("activity-elm-intakeairtemperature")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error)  {
	...
	device := context.GetInput("devicePath").(string)
        ...
        elmObj, err := dev.RunOBDCommand(elmobd.NewIntakeAirTemperature())

	if err != nil {
                log.Infof("Failed to get intake air temperature [%s]", err)
                return
        }

        log.Infof("Intake air temperature is [%s]", elmObj.ValueAsLit())
